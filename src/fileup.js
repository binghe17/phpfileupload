        ...
        if (input && input.type === 'file') {
            ...
            if (options.files.length > 0) {
                for (var i = 0; i < options.files.length; i++) {
                    ...
                    if (options.files[i].thumbnailUrl) {//previewUrl -> thumbnailUrl
                        tpl = tpl.replace(/\[PREVIEW_SRC\]/g, options.files[i].thumbnailUrl);
                    ...
                    if (options.files[i].url) { //downloadUrl -> url
                        tpl = tpl.replace(/\[FILE_SRC\]/g, options.files[i].url);
                        // var $name = $('#fileup-' + options.inputID + '-' + input.fileup.nextID).find('.fileup-name');
                        // if ($name[0]) {
                        //     $name.replaceWith('<a href="' + options.files[i].url + '" class="fileup-name" ' +
